          ...
          <>
            <div ref={fieldsRef} className="node-scroll-region" style={{ display: "flex", flexDirection: "column", gap: 7, overflowY: "auto", overscrollBehavior: "contain", paddingRight: 2, minHeight: 0, flex: 1 }}>
              ...
              {parsed.error ? (
                ...
                <>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 5, minHeight: 0 }} aria-label="Style profile JSON keys">
                    {parsed.fields.map((field) => (
                      <span key={field.id} title={field.value} style={{ color: config.accentText, background: config.accentBg, border: `1px solid ${config.accent}3a`, padding: "3px 7px", borderRadius: 4, fontSize: 10, fontFamily: "monospace" }}>
                        {field.type === "color" ? <span style={{ display: "inline-block", width: 8, height: 8, borderRadius: 2, background: isValidHexColor(field.value) ? normalizeHexColor(field.value) : "#666", marginRight: 4, verticalAlign: "middle" }} /> : null}
                        {field.key}
                      </span>
                    ))}
                  </div>
                ...
                <span style={{ color: "rgba(255,255,255,0.3)", fontSize: 10 }}>Type a JSON object to define profile keys.</span>
              )}
            </div>
          </>
        ) : (
                    ...
                    <div style={{ minWidth: 0, display: "flex", flexDirection: "column", gap: 5 }}>
                      ...
                      {field.type === "color" ? (
                        <div style={{ display: "flex", gap: 6 }}>
                          <input type="color" value={isValidHexColor(field.value) ? normalizeHexColor(field.value) : "#000000"} disabled={readOnly} aria-label={`${field.key || config.displayName} color picker`} onMouseDown={buttonMouseDown} onChange={(event) => updateField(field.id, { value: event.target.value.toUpperCase() })} style={{ width: 32, height: 28, border: "1px solid rgba(255,255,255,0.1)", borderRadius: 5, padding: 2, background: "rgba(0,0,0,0.16)", cursor: readOnly ? "default" : "pointer" }} />
